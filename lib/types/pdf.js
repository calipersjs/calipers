...
function measure (path, startBuffer) {

  // Get the offset of the xref table.
  var xrefOffset = getXrefOffset(startBuffer);

  // Get the offsets of every object in the PDF.
  var objectOffsets = getObjectOffsets(startBuffer, xrefOffset);
  ...
  for (var i = 0; i < objectOffsets.length; i++) {
    dict = getObjectDictionary(startBuffer, objectOffsets[i]);
    if (dict.cropBox) {
      return dict.cropBox;
